...
class Task extends Equatable {
  ...
  final String? room; // e.g. 'Badkamer', 'Keuken', 'Slaapkamer', 'Woonkamer'
  ...
  factory Task.fromMap(Map<String, dynamic> map) => Task(
    ...
    isHouseholdTask: map['isHouseholdTask'] is bool
        ...
        : (map['isHouseholdTask'] is String
              ...
              : false),
    householdId: map['householdId'] is String
        ? map['householdId'] as String
        : (map['householdId']?.toString()),
    reminderEnabled: map['reminderEnabled'] is bool
        ? map['reminderEnabled'] as bool
        : (map['reminderEnabled'] is String
              ? (map['reminderEnabled'].toLowerCase() == 'true')
              : false),
    reminderOffsetMinutes: () {
      final v = map['reminderOffsetMinutes'];
    ...
    serverVersion: () {
      // serverVersion may be provided directly as an int/string, or the
      // map may include a Firestore Timestamp under `serverUpdateTimestamp`
      // or `lastSyncedAt` which we should convert to millis for comparison.
      final v = map['serverVersion'];
      if (v is int) return v;
      if (v is String) return int.tryParse(v);
      final ts = map['serverUpdateTimestamp'] ?? map['lastSyncedAt'];
      try {
        if (ts is fs.Timestamp) return ts.millisecondsSinceEpoch;
