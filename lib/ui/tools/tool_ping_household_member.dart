...
class _SenderCommandCenterState extends State<_SenderCommandCenter> {
  ...
  bool get _isCooldownActive => _cooldownRemaining > Duration.zero;
  ...
  void _startCooldown() {
    ...
    setState(() {
      ...
      _cooldownMemberName = memberName;
    });
    ...
    _cooldownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (_cooldownRemaining <= const Duration(seconds: 1)) {
        timer.cancel();
        setState(() {
      ...
      setState(() {
        _cooldownRemaining = Duration(
          seconds: _cooldownRemaining.inSeconds - 1,
        );
      });
    });
  }
              ...
              children: [
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Cool-down active for ${_cooldownMemberName ?? _selectedMemberName ?? 'member'}: '
                            '${_formatDuration(_cooldownRemaining)} remaining.',
                            style: theme.textTheme.bodyMedium,
                          ),
                        ),
                      ],
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: _isCooldownActive || _selectedMemberId == null
                        ? null
                        : _startCooldown,
                    icon: const Icon(Icons.send),
                    label: Text(
                      _isCooldownActive
                          ? 'Wait ${_formatDuration(_cooldownRemaining)}'
