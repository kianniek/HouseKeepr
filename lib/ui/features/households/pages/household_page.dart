...
import '../../../common/widgets/assignee_dropdown.dart';
                ...
                Expanded(
                  child: BlocBuilder<ShoppingCubit, ShoppingState>(
                    builder: (context, state) {
                      ...
                      Widget buildItemRow(GroceryItem it) {
                        return Dismissible(
                          key: ValueKey(it.id),
                          ...
                          child: ListTile(
                            title: Text(it.name),
                            subtitle: it.category.displayName != 'Other'
                                ? Text(it.category.displayName)
                                : null,
                            leading: Checkbox(
                              value: it.checked,
                              onChanged: (v) {
                                final inCart = v ?? false;
                                context.read<ShoppingCubit>().toggleItem(it.id);
                                if (inCart) {
                                  setState(
              ...
              onPressed: canAdd
                  ? () {
                      ...
                      final cat = _cat.text.trim().isEmpty
                          ...
                          : GroceryCategory.fromString(_cat.text.trim());
                      context.read<ShoppingCubit>().addItem(
                        name,
                        category: cat,
                      );
