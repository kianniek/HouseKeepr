...
import '../../../common/widgets/simple_cropper.dart';
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    widget.apis.firestore
        ...
        .catchError((e) {
          debugPrint(
            'ProfilePage: failed to fetch user ${widget.user.uid}: $e',
          );
        });
  }
  ...
  Future<void> _pickAndUploadPhoto() async {
    ...
    try {
      ...
      if (widget.apis.skipImageProcessing) {
        ...
        // `path` is nullable on some platforms, but in this branch we are
        // not on web and `picked` was non-null. Use a non-null assertion so
        // the analyzer knows we expect a valid filesystem path here.
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      await widget.apis.firestore.collection('users').doc(uid).delete();
      ...
      if (mounted) {
        navigator.pop(); // Pop progress dialog
        setState(() => _saving = false);
      }
      if (e.code == 'requires-recent-login' && messenger != null) {
        messenger.showSnackBar(
          const SnackBar(
            content: Text(
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  enabledBorder: OutlineInputBorder(
                    borderSide: BorderSide(color: color ?? scheme.outline),
                  ),
