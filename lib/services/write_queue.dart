...
class WriteQueue {
  ...
  // --- ADDED METHOD ---
  /// Returns the set of task IDs that are currently queued for deletion.
  /// This helps other components (like TaskCubit) know which tasks should
  /// be hidden from the UI even if the server returns them.
  Set<String> getPendingDeleteTaskIds() {
    return _queue
        .where((op) => op.type == QueueOpType.deleteTask)
        .map((op) => op.id)
        .toSet();
  }
  // --------------------
